...
public class HashSetApplyEx {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < n; i++) { // n개의 개수 만큼 단어 입력 받기
            word[i] =sc.next();
        }

        // HashSet 을 이용해 중복 제거
        HashSet<String> hashSet = new HashSet<>(Arrays.asList(word));
        word = hashSet.toArray(new String[0]); // 배열의 크기를 0으로 지정하면 자동으로 배열 크기가 지정된다.
        Arrays.sort(word, new Comparator<String>() {
            @Override
            public int compare(String o1, String o2) {
                if(o1.length() == o2.length()) {
                    return o1.compareTo(o2); // 결과가 양수면, 정렬 조건 (o2가 사전 순으로 더 앞인 경우)
                } else {
